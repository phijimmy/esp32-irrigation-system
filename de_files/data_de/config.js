...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (restartBtn) {
        restartBtn.addEventListener('click', function() {
            ...
            restartBtn.textContent = 'Neustart läuft...';
            fetch('/api/restart', { method: 'POST' })
                .then(r => r.json())
                .then(data => {
                    if (data && data.result === 'ok') {
                        restartBtn.textContent = 'Neustart abgeschlossen!';
                        alert('Gerät wird neu gestartet...');
                    } else {
                        restartBtn.textContent = 'Neustart fehlgeschlagen';
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', function() {
            if (!confirm('Sind Sie sicher, dass Sie die Konfiguration löschen möchten? Dies entfernt config.json und stellt die Standardwerte nach dem Neustart wieder her.')) return;
            clearBtn.disabled = true;
            clearStatus.textContent = 'Konfiguration wird gelöscht...';
            fetch('/api/clearconfig', { method: 'POST' })
                .then(r => r.json())
                .then(data => {
                    if (data && data.result === 'ok') {
                        clearStatus.textContent = 'Konfiguration gelöscht. Bitte Gerät neu starten.';
                    } else {
                        clearStatus.textContent = data && data.error ? data.error : 'Konfiguration konnte nicht gelöscht werden.';
                    }
                })
                .catch(() => {
                    clearStatus.textContent = 'Konfiguration konnte nicht gelöscht werden.';
                })
                .finally(() => {
    ...
    if (setTimeBtn) {
        setTimeBtn.addEventListener('click', function() {
            ...
            if (!(year && month && day && hour >= 0 && minute >= 0 && second >= 0)) {
                statusDiv.textContent = 'Bitte alle Datums-/Zeitfelder ausfüllen.';
                return;
            }
            ...
            .then(resp => {
                if (resp && resp.result === 'ok') {
                    statusDiv.textContent = 'Gerätezeit gesetzt!';
                } else {
                    statusDiv.textContent = resp && resp.error ? resp.error : 'Zeit konnte nicht gesetzt werden.';
                }
            })
            .catch(() => {
                statusDiv.textContent = 'Zeit konnte nicht gesetzt werden (Netzwerkfehler).';
            })
            .finally(() => {
        ...
        .then(data => {
            ...
            if (schedLoading) {
                if (schedulePopulated) {
                    ...
                    schedLoading.textContent = 'Zeitplan konnte nicht geladen werden. Bitte Konfiguration prüfen oder neu laden.';
        ...
        newConfig.weekly_schedule = schedule;
            ...
            if (relaysContainer && typeof config.relay_count === 'number') {
                ...
                for (let i = 0; i < config.relay_count; i++) {
                    ...
                    html += `<fieldset style="margin-bottom:1em;"><legend>Relais ${i} Einstellungen</legend>`;
                    ...
                    html += `<label>Schaltzustand:</label> `;
                    ...
                    if (i === 0 && config.int_sqw_gpio !== undefined) {
                        html += `<div style='margin-top:0.5em; color:#888; font-size:0.95em;'><b>INT/SQW GPIO:</b> <span id='int-sqw-gpio-info'>${config.int_sqw_gpio}</span> <span style='font-style:italic;'>(nur Info)</span></div>`;
                    }
                    if (i === 2) {
